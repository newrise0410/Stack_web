...
export const TYPE_CODE = { Table: 'TBL', Pendant: 'PND', MoonWall: 'MWL', Tech: 'TEC', Clock: 'CLK', Shelf: 'SHF', Planter: 'PLN' };
...
const productSchema = new Schema(
  {
    ...
    category: { type: String, enum: ['Lighting', 'Tech', 'Clock', 'Shelf', 'Planter'], default: 'Lighting' },
    // 카탈로그의 단일 탐색 축(헤더 내비·카테고리 페이지가 이 값으로 필터).
    // Lighting → Table/Pendant/MoonWall, Tech → Tech, Clock → Clock, Shelf → Shelf, Planter → Planter.
    type: {
      type: String,
      enum: PRODUCT_TYPES,
      required: true,
